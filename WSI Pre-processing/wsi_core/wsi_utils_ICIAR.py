...
def savePatchIter_bag_hdf5(patch,i):
    x, y, cont_idx, patch_level, downsample, downsampled_level_dim, level_dim, img_patch, name, save_path, check_patches = tuple(patch.values())
...
def initialize_hdf5_bag(first_patch, save_coord=False):
    x, y, cont_idx, patch_level, downsample, downsampled_level_dim, level_dim, img_patch, name, save_path, check_patches  = tuple(first_patch.values())
    head_tail = os.path.split(name) 
